...
{
	/* 开启 GPIO 时钟 */
	rcu_periph_clock_enable(FLASH_SPI_GPIO_CLK);

	/* CS / SCK / MOSI：普通推挽输出 */
	GPIO_AF_INIT_OUT_PP(FLASH_SPI_CS_PORT,   FLASH_SPI_CS_PIN,   GPIO_OSPEED_50MHZ);
	GPIO_AF_INIT_OUT_PP(FLASH_SPI_SCK_PORT,  FLASH_SPI_SCK_PIN,  GPIO_OSPEED_50MHZ);
	GPIO_AF_INIT_OUT_PP(FLASH_SPI_MOSI_PORT, FLASH_SPI_MOSI_PIN, GPIO_OSPEED_50MHZ);

	/* MISO：上拉输入 */
	GPIO_AF_INIT_IPU(FLASH_SPI_MISO_PORT, FLASH_SPI_MISO_PIN);
...
uint8_t SPI1_ReadWriteByte(u8 dat)
{
	...
	for (i = 0; i< 8; i++)
	{
		...
		if (gpio_input_bit_get(FLASH_SPI_MISO_PORT, FLASH_SPI_MISO_PIN))
		{
			read_data |= 0x01;
		}
	}
