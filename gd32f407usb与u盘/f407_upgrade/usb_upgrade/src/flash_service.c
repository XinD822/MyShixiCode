...
static uint8_t SvcMutex_Lock(SvcMutex_t *m, uint32_t timeout_ms)
{
    ...
    while (1) {
        ...
        if ((Tick_GetMs() - start) >= timeout_ms) return 0;
        Tick_DelayMs(1);
    }
}
...
void FlashService_Read(uint8_t *buf, uint32_t addr, uint32_t len)
{
    ...
    if (cache_page == svc_cache_addr && svc_cache_dirty) {
        ...
        if (copy_len < len) {
            Flash_Read(buf + copy_len, addr + copy_len, len - copy_len);
        }
    } else {
...
void FlashService_Write(const uint8_t *buf, uint32_t addr, uint32_t len)
{
    ...
    while (len > 0) {
        ...
        if (cache_page != svc_cache_addr) {
            if (svc_cache_dirty) {
                Flash_EraseSector(svc_cache_addr);
                Flash_WriteNoCheck(svc_cache, svc_cache_addr, 4096);
                svc_cache_dirty = 0;
            }
            Flash_Read(svc_cache, cache_page, 4096);
