...
{
	/* 开启 GPIO 时钟（F103=APB2, F407=AHB1） */
	FLASH_SPI_CLK_CMD(FLASH_SPI_GPIO_CLK, ENABLE);

	/* CS / SCK / MOSI：普通推挽输出 */
	GPIO_AF_INIT_OUT_PP(FLASH_SPI_CS_PORT,   FLASH_SPI_CS_PIN,   GPIO_Speed_50MHz);
	GPIO_AF_INIT_OUT_PP(FLASH_SPI_SCK_PORT,  FLASH_SPI_SCK_PIN,  GPIO_Speed_50MHz);
	GPIO_AF_INIT_OUT_PP(FLASH_SPI_MOSI_PORT, FLASH_SPI_MOSI_PIN, GPIO_Speed_50MHz);
