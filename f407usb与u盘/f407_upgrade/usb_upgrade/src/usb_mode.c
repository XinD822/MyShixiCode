...
USB_Mode_t USB_Mode_ReadFlag(void)
{
    ...
    if (g_force_device) {
        DBG_PRINTF("[USB_MODE] *** RECOVERY: forcing DEVICE mode ***\r\n");
    ...
    if (flag == USB_MODE_FLAG_HOST) {
        DBG_PRINTF("[USB_MODE] Flash flag = HOST\r\n");
        return USB_MODE_HOST;
    }
    DBG_PRINTF("[USB_MODE] Flash flag = DEVICE (flag=0x%08X)\r\n",
               (unsigned int)flag);
    return USB_MODE_DEVICE;
}
...
void USB_Mode_WriteFlag(USB_Mode_t mode)
{
    ...
    DBG_PRINTF("[USB_MODE] Flash flag written: %s\r\n",
