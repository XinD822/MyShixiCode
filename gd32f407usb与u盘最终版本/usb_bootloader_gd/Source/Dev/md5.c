/**
 * @file md5.c
 * @brief MD5 消息摘要算法实现（RFC 1321 参考实现精简版）
 *
 * 标准 MD5，与同事 OTA 项目算法一致（同输入同摘要），
 * 用于固件完整性校验。支持流式计算：Init → Update×N → Final。
 * 本文件与 App 工程 usb_upgrade/src/md5.c 内容一致，保证两端摘要一致。
 */
...
/* 小端取 32 位 */
static uint32_t MD5_GetU32(const uint8_t *p)
{
    return (uint32_t)p[0]
         | ((uint32_t)p[1] << 8)
         | ((uint32_t)p[2] << 16)
...
static void MD5_PutU32(uint8_t *p, uint32_t v)
{
    p[0] = (uint8_t)(v);
    p[1] = (uint8_t)(v >> 8);
    p[2] = (uint8_t)(v >> 16);
...
void MD5_Final(uint8_t digest[16], MD5_CTX *ctx)
{
    ...
    MD5_PutU32(bits, ctx->total[0]);
    MD5_PutU32(bits + 4, ctx->total[1]);
    ...
    for (i = 0; i < 4; i++) {
        MD5_PutU32(digest + i * 4, ctx->state[i]);
    }
}
