/*
 * CherryUSB MSC Device - 支持 STM32F103 (FSDEV) 和 STM32F407 (DWC2 OTG)
 * Connects CherryUSB MSC to mass_mal.c
 */
...
/* ──── USB 寄存器基地址（根据芯片系列自动选择） ──── */
#if defined(CHIP_SERIES_F103)
  #define USB_REG_BASE    0x40005C00UL    /* STM32F103 USB_FSDEV */
#elif defined(CHIP_SERIES_F407)
  #define USB_REG_BASE    0x50000000UL    /* STM32F407 USB_OTG_FS */
#else
  #error "Unsupported chip series for USB"
#endif
...
static const char *string_descriptors[] = {
    ...
    "USB Upgrade Module",           /* Serial Number */
};
...
int usbd_msc_sector_read(uint8_t busid, uint8_t lun, uint32_t sector, uint8_t *buffer, uint32_t length)
{
    ...
    if (MAL_Read(lun, sector * 512, (uint32_t *)buffer, (uint16_t)length) != MAL_OK) {
        return -1;
    }
    return 0;
}
...
{
    g_last_scsi_tick = HAL_Tick->get_tick();
    if (MAL_Write(lun, sector * 512, (uint32_t *)buffer, (uint16_t)length) != MAL_OK) {
        return -1;
    }
    return 0;
}
...
void usb_msc_device_init(void)
{
    ...
    usbd_initialize(0, USB_REG_BASE, usbd_event_handler);
}
